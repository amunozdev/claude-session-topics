...
### Docs
- README's Install section now notes that installing opens the color picker on an interactive terminal (`Esc` keeps the current color; `--color <name>` or non-TTY/CI skips it).

## [5.2.3] — 2026-06-01

### Changed
- Installing now opens the interactive color picker immediately (on any interactive-terminal install, not only the first one), pre-selected to the current color. Press `Esc` to keep what's already set. Still skipped when `--color` is passed or on non-TTY/CI.
